    ...
    >
      <div className="container mx-auto px-2 sm:px-6 space-y-8 sm:space-y-12 text-[#ffe19b]">
        ...
        <div className="grid gap-6 sm:gap-10 grid-cols-1 md:grid-cols-2">
          {brands.map(brand => (
            <div key={brand.id} className="bg-white bg-opacity-50 rounded-lg shadow-lg p-3 flex flex-col md:flex-row items-center hover:shadow-2xl transition-shadow border-4 border-[#ffe19b] rounded-xl w-full max-w-4xl mx-auto"> {/* Reduced padding for shorter height */}
              <img src={brand.logo} alt={brand.name} className="w-36 h-24 sm:w-80 sm:h-36 mb-3 md:mb-0 md:mr-6 object-contain aspect-square rounded" /> {/* Reduced image height */}
              <div className="flex-1 flex flex-col items-center text-center md:items-start md:text-left">
                <h2 className="text-xl sm:text-2xl font-serif text-[#051905] mb-2">{brand.name}</h2>
                <p className="mb-3 text-[#051905] text-sm sm:text-base">{brand.description}</p>
